...
contract GrandPass is ERC1155, Ownable {
    ...
    uint256[] public COUNTS = [5, 5, 10, 10, 8, 1, 2, 8, 2, 5, 4, 5, 5, 5, 5, 2, 2, 6, 7, 3];

    address public dataFeedAddress = 0x71041dddad3595F9CEd3DcCFBe3D1F4b0a16Bb70; // Indirizzo fisso iniziale per il data feed 

    constructor(string memory uri_, string memory baseURI_) ERC1155(uri_) Ownable(msg.sender) {
        baseURI = baseURI_;
    ...
    function mint(address to) external {
        // Controllo dello stato di minting: 
        ...
        // Controllo del periodo di whitelist
        require((block.timestamp > launchTimestamp + WHITELIST_PERIOD) || whitelisted[to], "Not allowed user!");
        // Controllo del numero massimo di minting
        require(totalMintedCounts < TOTAL_COUNTS, "Minted all NFTs!");
        // Controllo dell'utente: Ogni utente può detenere un solo token
        require(minted[to] == false, "Can hold only one NFT!");

        // Generazione di un Token ID Randomico che utilizza tra le altre cose il prezzo di ETH/USD (scelta arbitraria) 
        uint256 _price = uint256(getPrice());
        uint256 xrand = uint256(keccak256(abi.encodePacked(_price, block.timestamp, to)));

        // Calcolo del Token ID: Ottiene un _tokenId casuale nell'intervallo tra 0 e 19 
        // Ci sono 20 asset RWA, cioè Real World Assets, definiti come possibili token ID.
        uint256 _tokenId = xrand % 20; // 20 RWA assets

        // Questa parte del codice serve per trovare un tokenId disponibile.
        // Se il tokenId scelto casualmente ha già raggiunto il numero massimo disponibile (mintedCounts[_tokenId] >= COUNTS[_tokenId]),
        // allora passa al successivo (_tokenId + 1) modulo 20 per assicurarsi che resti tra 0 e 19. 
        // Questo ciclo continua finché non trova un tokenId disponibile.
